...
sub search {
    ...
    $logger->tracef('searching %s for %s', 
        $self->{hostname}, $search_args);
...
sub search_for_containers {
    ...
    if (@containers) {
        ...
        my $base_cononical_dn_length = length(
            _canonical_dn(_exploded_dn($base)));
...
sub search_for_list {
	...
	my @entries;
	$self->search($search_args, sub {
	    my ($entry) = @_;
	    
	    push( @entries, $entry_mapper
	        ? &$entry_mapper($entry)
	        : $entry );
	});
...
=method canonical_dn($dn, %options)

Passes through to L<Net::LDAP::Util/canonical_dn>.

=method delete($base, %options)
...
=method exploded_dn($dn, %options)

Passes through to L<Net::LDAP::Util/ldap_explode_dn>.

=method export_ldif($to, %options)
...
=method import_ldif($from, %options)
...
=method search_for_map($search_args, $entry_mapper)
...
=method search_for_scalar($search_args, $entry_mapper)
